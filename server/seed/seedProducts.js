...
// exitProcess defaults to true so `npm run seed` connects on its own and
// exits cleanly when finished. server.js instead calls this with
// { exitProcess: false } to auto-seed an empty database right after it has
// already connected on startup, without killing the running process.
const importData = async ({ exitProcess = true } = {}) => {
  try {
    if (exitProcess) await connectDB();
...
const destroyData = async ({ exitProcess = true } = {}) => {
  try {
    if (exitProcess) await connectDB();
    await Product.destroy({ truncate: true });
...
if (require.main === module) {
  // Run directly via `npm run seed` / `npm run seed:destroy`
